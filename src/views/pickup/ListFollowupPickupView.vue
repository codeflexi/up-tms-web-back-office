...
const getPickings = async () => {
  try {
    const res = await axios.get(
      `/api/v1/shipment-picks?status=CREATED&sort=-planned_date&limit=25&page=${page.value}`
    );
  ...
  >
    ...
    <div class="mt-2 ml-10">
      <div class="users grid md:grid-cols-3">
        ...
        >
          <router-link :to="`/pickup/pickup-detail/${pick._id}`">
            <div>
              <h2>Picking#: {{ pick.pick_number }}</h2>
