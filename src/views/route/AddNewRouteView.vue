          ...
          >
            <div class="px-8 pt-4 pb-4 space-y-6">
              <div class="flex justify-between items-start mb-16">
                <div>
                  <h1 class="text-4xl font-bold">ADD NEW DELIVERY</h1>
                  <p class="text-gray-600">เพิ่มรายการ สำหรับส่งสินค้า</p>
                </div>
              ...
              <div>
                <label class="mb-1 block" for="title">From Source</label>
                <select v-if="routeStore.getBranch"
                  class="form-select px-4 py-3 rounded-xl w-full ring-red-700"
                  name="gender-selection"
                  @change="onSelectChange(e)"
                  v-model="form_input.selectedValueFrom"
                >
                  <option
                    v-for="item in routeStore.getBranch.data"
                    :value="item._id"
                    :key="item._id"
                  >
                    {{ item.name }}
                ...
                <p>Selected value: {{ form_input.selectedValueFrom }}</p>
              </div>
            
              <div>
                <label class="mb-1 block" for="title">To Destination</label>
                <select v-if="routeStore.getBranch"
                  class="form-select px-4 py-3 rounded-xl w-full ring-red-700"
                  name="gender-selection"
                  @change="onSelectChange(e)"
                  v-model="form_input.selectedValueTo"
                >
                  <option
                    v-for="item in routeStore.getBranch.data"
                    :value="item._id"
                ...
                <p>Selected value: {{ form_input.selectedValueTo }}</p>
              </div>
