<template>
        ...
        <div class="flex items-center justify-between px-1.5 py-0.5">
          <div class="flex">
            <button
              ...
              type="button"
              class=" ml-5 py-2 px-2 focus:outline-none text-white rounded mt-4 mb-4 flex items-center  bg-black"
            >
  ...
  import Modal from "@/views/shipment/DeleteShipmentModal.vue";
  ...
  const getShipments = async () => {
    try {
      ...
      const res = await axios.get(
       ...
       `/api/v1/shipments?status=DATA SUBMITTED&limit=100&page=${page.value}${filter}${filter1}${filter2}${filter3}`
      );
